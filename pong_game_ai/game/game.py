...
def draw(window, paddles, ball, left_score, right_score):
    ...
    window.blit(left_score_text, (WIN_WIDTH//4 - left_score_text.get_width()//2, 20))
    window.blit(right_score_text, (WIN_WIDTH * (3/4) - right_score_text.get_width()//2, 20))

    for paddle in paddles:
        paddle.draw(window)

    for i in range(10, WIN_HEIGHT, WIN_HEIGHT//20):
        pygame.draw.rect(window, WHITE, (WIN_WIDTH//2 - 5, i, 10, WIN_HEIGHT//40))
...
def paddle_movement(keys, left_paddle, right_paddle):
    ...
    if keys[pygame.K_s] and left_paddle.y + left_paddle.VEL + left_paddle.height <= WIN_HEIGHT:
        left_paddle.move(up=False)
    
    if keys[pygame.K_UP] and right_paddle.y - right_paddle.VEL >= 0:
        right_paddle.move(up=True)
    if keys[pygame.K_DOWN] and right_paddle.y + right_paddle.VEL + right_paddle.height <= WIN_HEIGHT:
